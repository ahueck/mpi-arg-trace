...
void mpi_arg_trace_push_full(
  ...
  const MPI_Comm* COMM_COMPARISON,
  const int* COMM_SIZE,
  ...
  const MPI_File* FILE,
  const MPI_Win_copy_attr_function* FUNCTION,
  ...
  const int* POLYDTYPE_STRIDE_BYTES,
  const MPI_Datarep_conversion_function* POLYFUNCTION,
